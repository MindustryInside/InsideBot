...
public class DurationFormat {
    ...
    public static DurationFormatter wordBased(Locale locale) {
        return FORMATTERS.computeIfAbsent(locale, k -> {
            DynamicWordBased dynamic = new DynamicWordBased(buildWordBased(locale));
            return new DurationFormatter(dynamic, locale);
        });
    }
    ...
    static class DynamicWordBased implements DurationPrinter {
        ...
        public int countFieldsToFormat(TemporalAmount temporalAmount, int stopAt, Locale locale) {
            return getPrinter(locale).countFieldsToFormat(temporalAmount, stopAt, locale);
        }

        @Override
        public int calculateFormattedLength(TemporalAmount temporalAmount, Locale locale) {
            return getPrinter(locale).calculateFormattedLength(temporalAmount, locale);
        }

        @Override
        public void formatTo(StringBuilder buf, TemporalAmount temporalAmount, Locale locale) {
            getPrinter(locale).formatTo(buf, temporalAmount, locale);
        }
